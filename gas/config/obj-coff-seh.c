...
static void
seh_arm64_add_unwind_element (seh_arm64_unwind_types unwind_type, int offset, int reg)
{
  if (seh_ctx_cur == NULL
  ...
  const struct unwind_code_pack_info *unwind_code_pack_info =
    unwind_code_pack_infos + unwind_type;
...
obj_coff_seh_save_reg (int type)
{
  ...
  const struct unwind_code_pack_info *unwind_code_pack_info =
    unwind_code_pack_infos + type;

  if (!unwind_code_pack_info->directive
      || !verify_context_and_target (unwind_code_pack_info->directive, seh_kind_arm64)
